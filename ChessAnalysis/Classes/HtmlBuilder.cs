...
namespace ChessAnalysis.Classes
{
	public static class HtmlBuilder
	{
		...
		{
			return string.Format(TABLE_ROW, string.Empty
				.AppendHeaderCell("Round")
				.AppendHeaderCell("Next player")
				.AppendHeaderCell("White castling")
				.AppendHeaderCell("Black castling")
				.AppendHeaderCell("En passant")
				.AppendHeaderCell("Half moves")
				.AppendHeaderCell("Best move")
				.AppendHeaderCell("Result")
				.AppendHeaderCell("Opening")
				.AppendHeaderCell("Defense")
				.AppendHeaderCell("Players"));
		}

		private static string BuildTableRow(Data data)
		{
			return string.Format(TABLE_ROW, string.Empty
				.AppendCell(data.Position.Round)
				.AppendCell(data.Position.NextPlayer)
				.AppendCell(data.Position.CastlingWhite)
				.AppendCell(data.Position.CastlingBlack)
				.AppendCell(data.Position.EnPassant)
				.AppendCell(data.Position.HalfMoves)
				.AppendCell(data.Position.BestMove)
				.AppendCell(data.Comment.Result)
				.AppendCell(data.Comment.Opening)
				.AppendCell(data.Comment.Defense)
				.AppendCell(data.Comment.Players));
		}
